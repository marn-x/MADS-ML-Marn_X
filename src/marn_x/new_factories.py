...
class ConfigurableDatasetFactory(AbstractDatasetFactory[ImgDatasetSettings]):
    ...
    def __init__(
        ...
        super().__init__(settings, datadir=datadir)
        self.config = config
    ...
    def create_dataset(
        ...
        **kwargs: Any
        ...
        paths_, class_names = iter_valid_paths(
            self.subfolder / self.config.subfolder_name,
...
class EurosatDatasetFactory(ConfigurableDatasetFactory):
    ...
    def __init__(self, settings: ImgDatasetSettings, datadir: Optional[Path] = None):
        ...
        config = DatasetConfig(
            ...
            formats=settings.formats if settings.formats is not None else [FileTypes.JPG],
...
def register_default_datasets():
    ...
    try:
        # from marn_x.main import DataLoaderFactory
        ...
        DatasetRegistry.register_dataset("eurosat", create_eurosat_loaders)
