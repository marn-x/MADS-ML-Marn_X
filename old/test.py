...
class TrainingConfig:
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'TrainingConfig':
        ...
        if unexpected:
            print(f"Warning: Ignoring unexpected configuration fields: {unexpected}")
...
class ConfigLoader:
    ...
    def load_config(cls, config_path: Optional[str] = None) -> TrainingConfig:
        ...
        if config_path is None:
            config_path = os.getenv("TRAINING_CONFIG_PATH", "config.yaml")
    ...
    **override_kwargs
    ...
    try:
        if config.use_mltrainer:
            ...
            # Run training
            print(f"Starting mltrainer with {config.backend} tracking...")
            ml_trainer.loop()
    ...
    def map_search_params(cls, search_params: Dict[str, Any], base_config: TrainingConfig) -> Dict[str, Any]:
        ...
        for search_key, search_value in search_params.items():
            ...
            if "_" in config_key and config_key.split("_")[0] in ["optimizer", "scheduler", "criterion"]:
                ...
                print(f"Warning: Unknown parameter {search_key} (mapped to {config_key})")
...
def ray_tune_pytorch(
    ...
    **tune_kwargs
    ...
    def train_ray_tune(search_params, checkpoint_dir=None):
        ...
        print(f"Ray Tune search params: {search_params}")
        ...
        print(f"Mapped config dict: {trial_config_dict}")
    ...
    print(f"Best trial final {config.ray_metric}: {best_trial.last_result[config.ray_metric]}")
...
if __name__ == "__main__":
    ...
    print("Example 1: Configuration-based training")
    ...
    # Save to YAML
    sample_config.to_yaml("config.yaml")
    ...
    try:
        ...
        print("Ray Tune not installed. Skipping example 6.")
